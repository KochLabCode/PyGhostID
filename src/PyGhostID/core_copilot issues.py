...
def ghostID(model, params, dt, trajectory, epsilon_Qmin=0.05, epsilon_SN_ghosts=0.1, **kwargs):
    ...
    ev_outlier_removal_ws   - Size of sliding window for outlier removal in eigenvalue time-series.
    ev_outlier_removal_k    - Size of the filter for outlier removal in eigenvalue time-series. 
                              Values outsisde +/- k*interquartile ranges are removed.                       

    Version 0.9
    """
    
    # Parse and validate kwargs
    config = parse_ghostid_kwargs(**kwargs)
    ...
    if len(idx_minima) > 0:
        ...
        for i in idx_minima:
            ...
            if leaves_eps_qmin_i:
                ...
                ev_signChanges = [sign_change(eigVals_real[:, ii],ev_outlier_removal_ws,ev_outlier_removal_k) for ii in range(n)]
...
def ghostID_phaseSpaceSample(model, model_params, t_start, t_end, dt, state_ranges,
    ...
    # Extract parameters from config
    peak_kwargs = config['peak_kwargs']
    ctrlOutputs = config['ctrlOutputs']
    model_batch = config['batchModel'] if config['batchModel'] is not None else make_batch_model(model, model_params)
...
def track_ghost_branch(ghost, model, model_params, par_nr, par_steps, dpar, t_end, dt, delta=0.5, icStep=0.1, mode="first",
    ...
    with tqdm(total=par_steps + 1) as pbar:
        while i < par_steps+1:
            ...
            gid_output = ghostID(model, model_params_, dt, sol.y.T,
                            epsilon_gid, evLimit=evLimit, slopeLimits=slopeLimits, peak_kwargs=peak_kwargs,
                            batchModel=batchModel, ctrlOutputs=ctrlOutputs, eigval_NN_sorting=eigval_NN_sorting,
